...
export async function sendAsnEmail({ trailer, pallets, jobs = [], recipients }) {
  ...
  const fromAddress = "Frive Outbound <onboarding@resend.dev>";
  console.log("[ASN] Sending via Resend", { from: fromAddress, to: toAddresses, subject, rows: rows.length, attachmentBytes: base64Csv.length });
  ...
  if (!res.ok) {
    ...
    console.error("[ASN] Resend API error", { status: res.status, body: err });
